...
import { analyzeRequirementDocument, analyzeRequirementScreenshot, clearRequirementAnalysis, confirmRequirementAnalysis, createRequirementDocument, deleteRequirementDocument, identifyRequirementLinkages, listRequirementDocuments, listRequirementModelOptions, parseRequirementDocument } from '../api/requirements'
...
const reviewedTestPointIds = ref([]); const reviewedEvidenceIds = ref([]); const reviewedAnalysisItemIds = ref([])
...
const analysisReview = computed(() => {
  ...
  if (!analysis || analysis.quality_status !== 'needs_review') return null
  ...
  return { uncoveredEvidence, uncitedItems, reviewPoints, mappingGapGroups, drops, totalEvidence: coverage.total_evidence || 0, coveredEvidence: coverage.covered_evidence || 0 }
})
...
const modelVerificationLabel = computed(() => {
  const report = selectedDocument.value?.latest_analysis?.coverage_report || {}
  const qualityStatus = report.quality_status || selectedDocument.value?.latest_analysis?.quality_status
  return qualityStatus === 'complete' ? '调用与完整性均通过' : '调用已验证，结果待复核'
})
...
async function confirmAnalysis() { if (!selectedDocument.value || !analysisReview.value || !reviewReady.value || !canWrite.value) return; busy.value = true; error.value = ''; failureMeta.value = null; try { await confirmRequirementAnalysis(selectedDocument.value.id, { reviewed_test_point_ids: reviewedTestPointIds.value, reviewed_evidence_ids: reviewedEvidenceIds.value, reviewed_analysis_item_ids: reviewedAnalysisItemIds.value }); notice.value = '分析结果已确认，现在可以进入用例生成。'; await loadDocuments(); selectedDocument.value = documents.value.find(item => item.id === selectedDocument.value?.id) || selectedDocument.value } catch (err) { error.value = explain(err, '分析确认失败，请重试。') } finally { busy.value = false } }
...
watch(() => selectedDocument.value?.id, () => { visibleTestPointCount.value = testPointPageSize; reviewedTestPointIds.value = []; reviewedEvidenceIds.value = []; reviewedAnalysisItemIds.value = []; screenshotReport.value = selectedDocument.value?.visual_analysis_report || null; loadModelOptions() })
watch(busy, (value) => {
  if (value) {
  ...
  <WorkspaceShell active="requirements"><div class="workspace-content requirement-page">
    ...
    <section v-if="analysisReview" class="requirement-review-panel" aria-labelledby="analysis-review-title">
      ...
      <div class="requirement-review-grid">
        ...
        <article class="requirement-review-points-card"><h3>最终测试点（主要审核对象）<small>{{ analysisReview.reviewPoints.length }} 条</small></h3><p class="requirement-review-description">按测试点清单逐行审核：确认它属于哪个模块和功能点、描述是否准确、测试类型是否合理。勾选序号表示该测试点已核对；全部通过后才会作为用例生成输入。</p><div v-if="analysisReview.reviewPoints.length && !analysisReview.mappingGapGroups.length" class="requirement-review-points-table-wrap"><table class="requirement-review-points-table"><thead><tr><th>审核</th><th>序号</th><th>功能模块</th><th>功能点</th><th>测试点描述</th><th>测试类型</th><th>原文依据</th></tr></thead><tbody><tr v-for="(item, index) in analysisReview.reviewPoints" :key="item.id"><td><input type="checkbox" :aria-label="`确认第${index + 1}条测试点`" :value="String(item.id)" v-model="reviewedTestPointIds"></td><td>{{ index + 1 }}</td><td>{{ item.moduleLabel }}</td><td>{{ item.functionLabel }}</td><td>{{ item.description || item.scenario || '未命名测试点' }}</td><td><span class="requirement-review-type">{{ item.typeLabel }}</span></td><td>{{ item.evidenceLabels.length ? item.evidenceLabels.join('；') : '无证据引用' }}</td></tr></tbody></table></div><p v-else-if="!analysisReview.mappingGapGroups.length" class="requirement-empty requirement-empty--compact">当前分析没有可审核的测试点，不能进入用例生成。</p><p v-else class="requirement-review-invalid-points">当前结果尚未形成可审核的最终测试点清单。请先重新执行深度分析，生成完整的模块名称、功能点名称、测试点描述和测试类型；这里不需要手工补写编号。</p></article>
      </div>
      ...
      <div class="requirement-review-actions"><button class="primary-action" :disabled="busy || !canWrite || !reviewReady" @click="confirmAnalysis">确认审核并允许生成用例</button><button class="secondary-action" :disabled="busy || !canWrite" @click="action(analyzeRequirementDocument, '需求深度分析完成')">重新执行深度分析</button><small>{{ analysisReview.mappingGapGroups.length ? '请先重新执行深度分析，补齐模块归属和测试类型后再审核。' : reviewReady ? '审核清单已逐项确认。' : '请勾选上方所有证据、分析项和最终测试点后再确认。' }} 确认不会删除需求原文或证据。</small></div>
    </section>
