...
const blockedDocuments = computed(() => documents.value.filter(item => item.latest_analysis && item.latest_analysis.quality_status !== 'complete'))
const selectedAnalysisDocument = computed(() => documents.value.find(item => item.id === selectedDocument.value))
const reviewDone = computed(() => Number(selectedRecord.value?.review_rounds || 0) >= 5 && Boolean(selectedRecord.value?.review_report?.analysis_method) && selectedRecord.value?.review_report?.execution_status !== 'failed')
...
function qualityLabel(status) { return { complete: '可生成', needs_review: '待确认', partial: '部分完成', failed: '分析失败' }[status] || '未完成' }
...
async function load() {
  ...
  try {
    ...
    if (!analyzedDocuments.value.some(item => item.id === selectedDocument.value)) selectedDocument.value = analyzedDocuments.value[0]?.id || ''
...
async function runGenerateWithStatus() {
  ...
  try {
    const qualityStatus = selectedAnalysisDocument.value?.latest_analysis?.quality_status
    if (qualityStatus && !['complete', 'needs_review'].includes(qualityStatus)) {
      error.value = `当前分析为${qualityLabel(qualityStatus)}，请返回需求分析页重新执行。`
      return
    }
    if (qualityStatus === 'needs_review') {
      error.value = '当前分析仍待人工审核，请先进入“需求智能分析”页面逐项核对测试点和证据，再确认后生成用例。'
      return
    }
    const result = await generateCaseRecord(selectedRecord.value?.document || selectedDocument.value, generationModel.value); notice.value = '五轮递进用例生成完成。'; await load(); selectedRecord.value = records.value.find(item => item.id === result.id) || result
...
async function runReviewWithStatus() {
  ...
  try { const result = await reviewCaseRecord(selectedRecord.value.id, reviewModel.value); notice.value = '五轮用例评审完成。'; await load(); selectedRecord.value = records.value.find(item => item.id === result.id) || result } catch (err) { error.value = explain(err, '用例评审失败，请检查模型路由或重试。') } finally { endBusy() }
}

async function runSelectWithStatus() {
  beginBusy('正在计算自动化适配性', 'select'); error.value = ''; notice.value = ''
  try { const result = await selectCaseRecord(selectedRecord.value.id); notice.value = '自动化筛选完成。'; await load(); selectedRecord.value = records.value.find(item => item.id === result.id) || result } catch (err) { error.value = explain(err, '自动化筛选失败，请重试。') } finally { endBusy() }
}
...
<template>
  <WorkspaceShell active="case-generation"><div class="workspace-content case-generation-page">
    ...
    <div v-if="!loading && selectedAnalysisDocument" class="case-generation-source-status" role="status"><b>分析状态：{{ qualityLabel(selectedAnalysisDocument.latest_analysis.quality_status) }}</b><span>模块 {{ selectedAnalysisDocument.latest_analysis.modules?.length || 0 }} · 功能点 {{ selectedAnalysisDocument.latest_analysis.functions?.length || 0 }} · 测试点 {{ selectedAnalysisDocument.latest_analysis.test_points?.length || 0 }}</span><small v-if="selectedAnalysisDocument.latest_analysis.quality_status === 'needs_review'">{{ qualityReason(selectedAnalysisDocument) }}；请先到需求智能分析页面完成逐项审核。</small><small v-else-if="selectedAnalysisDocument.latest_analysis.quality_status !== 'complete'">{{ qualityReason(selectedAnalysisDocument) }}；请回到需求分析页重新执行。</small></div>
    ...
    <section v-else class="case-generation-layout"><aside class="case-generation-panel"><div class="case-generation-heading"><h2>生成记录</h2><span>{{ records.length }}</span></div><div v-if="!records.length" class="case-generation-empty">暂无生成记录，请选择已完成分析的需求文档生成用例。</div><button v-for="item in records" :key="item.id" class="case-generation-record" :class="{ active: item.id === selectedRecord?.id }" @click="selectRecord(item)"><b>{{ item.document_title }}</b><small>{{ item.status }} · {{ item.total_cases }} 条</small></button></aside>
      <main class="case-generation-panel">
        <div v-if="!selectedRecord" class="case-generation-empty">
          <h2>生成测试用例</h2>
          <p>选择已有需求分析的文档后开始；待确认结果必须先在需求智能分析页面完成审核。</p>
          <select v-model="selectedDocument">
            <option value="">请选择需求文档</option>
            <option v-for="item in analyzedDocuments" :key="item.id" :value="item.id">
              {{ item.title }} v{{ item.version }} · {{ qualityLabel(item.latest_analysis.quality_status) }}
            </option>
          </select>
          ...
          <button class="primary-action" :disabled="busy || !canWrite || !selectedDocument" @click="runGenerate">{{ busy ? '生成中…' : '生成用例' }}</button>
          <p v-if="!analyzedDocuments.length" class="case-generation-hint">当前项目没有已分析需求文档。</p>
        </div>
        <template v-else>
          <div class="case-generation-heading"><div><h2>{{ selectedRecord.document_title }}</h2><small>生成 {{ selectedRecord.rounds }} 轮 · {{ selectedRecord.status }}</small></div><div class="case-generation-actions"><button class="secondary-action" :disabled="busy || !canWrite" @click="runGenerate">重新生成</button><button class="secondary-action" :disabled="busy || !canWrite || reviewDone" @click="runReview">{{ reviewDone ? '已评审' : '评审' }}</button><button class="secondary-action" :disabled="busy || !canWrite || selectionDone" @click="runSelect">{{ selectionDone ? '已筛选' : '自动化筛选' }}</button></div></div>
          ...
          <div class="case-generation-summary"><span>总用例 {{ selectedRecord.total_cases }}</span><span>自动化 {{ selectedRecord.auto_cases }}</span><span>手工 {{ selectedRecord.manual_cases }}</span><span>覆盖度 {{ Math.round((selectedRecord.coverage_report?.coverage_rate || 0) * 100) }}%</span></div>
          ...
          <div v-if="selectedRecord.review_report?.analysis_method" class="case-generation-review"><b>评审方式：{{ selectedRecord.review_report.analysis_method === 'model_verified' ? '模型复核' : '确定性基线' }}</b><span v-if="selectedRecord.review_report.issue_count !== undefined">问题 {{ selectedRecord.review_report.issue_count }} 条</span><p v-if="selectedRecord.review_report.model_warning" class="case-generation-error">模型评审未完成：{{ selectedRecord.review_report.model_warning }}</p><ul v-if="selectedRecord.review_report.issues?.length"><li v-for="(issue, index) in selectedRecord.review_report.issues" :key="issue.id || `review-issue-${index}`">【{{ issue.severity_label || ({ high: '高', medium: '中', low: '低' }[issue.severity] || '中') }}】{{ issue.description }} · {{ issue.suggestion }}</li></ul></div>
          <p v-if="selectedRecord.review_report?.approved === false" class="case-generation-error">评审未通过，请根据问题修正后重新生成。</p><div v-if="!selectedRecord.cases?.length" class="case-generation-empty">当前记录没有可展示的用例。</div><template v-else><table class="case-generation-table"><thead><tr><th>编号</th><th>标题</th><th>类型</th><th>优先级</th><th>自动化</th><th>预期结果</th></tr></thead><tbody><tr v-for="item in shownCases" :key="item.id"><td>{{ item.id }}</td><td>{{ item.title }}</td><td>{{ item.type }}</td><td>{{ item.priority }}</td><td>{{ item.automatable ? '建议自动化' : '建议手工' }}</td><td>{{ item.expected_result }}</td></tr></tbody></table><button v-if="hasMoreCases" class="secondary-action case-generation-more" @click="visibleCases += 100">加载更多（已显示 {{ shownCases.length }} / {{ selectedRecord.cases.length }}）</button></template>
        </template>
      </main>
    </section>
    <div v-if="!selectedRecord && blockedDocuments.length" class="case-generation-hint">存在待复核或未完整的需求分析结果，请先在需求分析页完成复核后再生成用例。</div>
