...
- 归档状态：当前开发进度已完成并验收 T170。T168 蓝图生命周期与 T167 配置回归已实现并通过浏览器验收；T169 统一运行时与业务入口整改已实现并通过自动化/浏览器验证；本地模型出网启动整改已获用户验收；T170 需求分析→需求评审→需求拆解 Skill 纵向闭环已实现并获用户验收；分析冲突中文核对体验优化已实现并通过真实浏览器验证，仍作为独立体验改进项保留。针对“同一来源重复分析结果波动过大、用户无法判断哪一版可信”的 P0 问题，已新增并登记 T170A，但尚未开发；T171 测试点拆解/评审与人工测试点审核必须等待 T170A 完成，完整需求到用例父级 DAG 尚未开始。
- 下次续接指令：用户在新 Codex 会话输入“继续”时，先按项目规则读取完整契约文档链、复核 Git 与本地服务，并核对唯一标记 `AITS-RESUME-20260924-ARCHIVE-T170-UX-NETWORK-9f3a7c21`；确认 T170 仍已验收后，按任务清单只开发新增前置任务 T170A，不得直接跳到 T171。网络整改和 T170 不再作为待验收项。不得提交、推送、部署或回滚。
- 工作区边界：保留当前全部未提交修改及既有未跟踪资料，禁止清理或覆盖；仅开发一个任务。下次启动必须重新检查服务和 Git 状态，不能假定当前进程仍在运行。
- 凭据边界：登录凭据不写入归档、代码或日志。

Updated: 2026-09-24 (Asia/Shanghai) — 网络整改与 T170 均已获用户验收；因分析结果稳定性风险，已新增 P0 前置任务 T170A，当前下一任务为 T170A，尚未开始。唯一续接标记保持不变。

## 2026-09-24 各业务域独立总运行与动态链路契约补充（仅文档，未改代码）

- **用户确认的核心要求：** 统一运行协议不等于一个混合所有测试类型的总运行。需求分析、接口自动化、UI 自动化、App 自动化、性能测试等功能页面必须各自拥有独立的 `workflow_key`/`domain` 父级总运行；共享的是状态机、快照、血缘、进度投影和失败恢复能力。
- **结果边界：** 不同业务域默认不能互相挂载节点、共享百分比分母或按“最新记录”混用结果。只有用户明确发起跨域编排时，才通过显式版本化产物和父子运行关系关联；每个领域仍保留自己的输入、节点、输出和完成条件。
- **文档落点：** PRD 新增各业务域独立总运行、唯一结果与动态进度要求；技术架构补充 `workflow_key`/`domain`、独立父级运行、跨域边界和领域执行器；TASKS 增加适用于 T169B、T170-T173、T173A 及 T178-T184 的共同门禁。
- **任务影响：** T170 的用户验收仍有效，但只代表需求分析/需求评审/需求拆解独立节点闭环；各业务域自己的总运行、动态进度和跨领域隔离仍必须按新增门禁补齐。
- **当前边界：** 本次仅更新 PRD、技术架构、任务清单和项目状态，不修改业务代码、不改变唯一归档标记、不提交、不推送、不部署、不回滚。

## 2026-09-24 总运行与动态链路自动评审结果（仅文档，未改代码）

- **审查结论：** 原有补充的主干方向正确，但仅写主流程和少量异常不足以支撑“结果唯一、不可混用、失败不放行”。自动交叉审查补充了输入/Schema、Skill/执行器就绪、模型供应商、环境/设备、人工门禁、幂等并发、取消恢复、Worker/队列/数据库故障、事件乱序、上游失效、外部副作用、成本熔断、权限中途变化、安全隐私、旧数据和用户恢复等场景。
- **正式结果：** `doc/TASKS_V5.0.md` 新增“总运行与动态链路自动评审结果矩阵”和默认安全决策，作为 PRD/技术架构之外的开发与验收最低证据集。
- **未完成声明：** 这是文档级审查和门禁补齐，不代表代码已经实现；后续每个业务域必须补充自己的节点、输入/输出 Schema、适用性、权重、重试边界、外部副作用清理和质量阈值，并按矩阵完成真实 API、前端和浏览器证据。
- **归档边界：** T170 用户验收和网络整改验收不变；唯一归档标记不变；本次不改代码、不改变当前下一任务记录、不提交、不推送、不部署、不回滚。

## 2026-09-24 平台级总运行与动态链路文档评审（用户通过）

- 用户已评审通过各业务域独立总运行、跨域隔离、唯一权威结果、动态进度、异常阻断、失败恢复和自动评审矩阵。
- 文档基线正式生效：后续 T169B、T171-T177 及阶段30相关业务域必须遵守该矩阵；未满足通用门禁或领域参数未定义时不得标记完成。
- 本次仅确认文档评审结果，不改代码、不改变 T170 验收状态、不改变唯一归档标记、不提交、不推送、不部署、不回滚。

## 2026-09-24 T170A 跨运行分析稳定性整改任务登记（待开发）

- **触发问题：** 同一需求重复深度分析后，待人工核对、测试点和证据覆盖等结果出现明显数量波动；现有实现会把新分析作为最新结果展示，但没有完整保存跨运行对象身份、权威快照和人工决策继承关系。该现象不能解释为“模型有随机性”后继续推进，因为用户无法判断新旧结果的可信边界。
- **判断结论：** 主要是结果治理和框架契约不完整，模型非确定性只是放大因素。T170 已验收的独立 Skill 节点闭环不撤销，但其结果不能直接视为跨运行稳定；T170A 是 T171 的 P0 前置任务。
- **文档整改：** PRD 新增跨运行分析稳定性、权威快照、稳定对象身份、差异分类、疑似删除和下游冻结要求；技术架构新增候选运行/权威快照/差异报告边界；TASKS 新增 T170A、详细 DoD、最小验收矩阵并调整 T171/T173A/T176/T177 依赖。
- **实现边界：** T170A 后续必须覆盖稳定证据/对象 ID、候选与权威版本分层、对象级差异、审核状态继承、关键变化阻断、下游精确快照引用、旧数据只读兼容、幂等并发和失败恢复；未完成前不得把“最新分析”作为 T171 或后续领域资产的唯一输入。
- **当前状态：** 本次仅登记整改要求和任务，未修改业务代码、未执行 T170A、未提交、未推送、未部署、未回滚；唯一归档标记保持不变。
...
- **当前边界与下一步：** T170 已获用户验收；因新增 P0 稳定性整改，先执行 T170A，T171 测试点拆解/评审与人工确认、T172 用例编写以及 T173+完整 DAG 尚未开始。保留 T167/T168/T169 既有未提交修改；不提交、不推送、不部署、不回滚。
...
- **部署结果：** 已将提交 `72470e065c966e1ef62f046ef1c183f16d3d528f` 部署到 `124.222.221.128`；用户入口 `http://124.222.221.128:8090/`，后端端口 `8088`。后端、Celery Worker、Celery Beat、PostgreSQL、Redis 均已运行；远程提交标记 `/opt/aits-v5/.deployed-commit` 与本地一致。
- **迁移与运行检查：** 生产必填 `PUBLIC_APP_URL` 已补为目标用户入口；数据库迁移成功，Django `check` 无问题，Celery `inspect ping` 返回 `pong`；服务器本机访问后端健康接口、前端入口和前端反代健康接口均 HTTP 200。
- **构建方式：** 远程复用既有依赖基础镜像并叠加本次提交代码生成 `aits-v5-backend:deploy-72470e0` 后切换为 `latest`；未改变依赖文件，未覆盖 `.env.remote` 中既有密钥、数据库或 Redis 数据卷。完整基础镜像重建因远程构建容器下载源速率异常缓慢未采用。
- **备份与回滚：** 原后端目录保留为 `/opt/aits-v5/backend.backup-20260918-182002`，原前端静态目录保留为 `/opt/aits-v5-validation-web/dist.backup-20260918-182002`；本次未执行回滚。远程 `.env.remote` 和持久化数据卷未删除。
- **外部验证边界：** 本机到远程公网 IP 的 HTTP 请求被当前执行环境网络策略拦截；服务器自身通过公网 IP 回环请求均为 200，因此已完成服务器侧部署验收，但未宣称本机外网连通证据或 Browser MCP 用户验收。Browser MCP 当前仍不可用。
- **Git 交付：** 部署所依据的 `72470e0` 已推送到 `origin/master`；本次部署记录写入状态文档后需再提交并推送，供下次会话续接。

## 2026-09-18 开发会话归档（下次直接接续）

- **会话状态：** 本次开发到此结束，未执行新的业务编码；归档后按明确指令完成一次远程部署，并补充了生产入口配置与部署状态记录。
