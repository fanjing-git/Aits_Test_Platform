...
async function runGenerateWithStatus() {
  ...
  try { const result = await generateCaseRecord(selectedRecord.value?.document || selectedDocument.value, generationModel.value); notice.value = '五轮递进用例生成完成。'; await load(); selectedRecord.value = records.value.find(item => item.id === result.id) || result } catch (err) { error.value = explain(err, '用例生成失败，请检查模型路由或重试。') } finally { endBusy() }
}
