...
| **文档版本** | V5.0（最终定稿） |
| **文档状态** | 已定稿 |
| **技术栈** | Python 3.12（已确认）+ Django 5.0 + LangChain 0.3 + LangGraph 0.2 +（LangSmith 可选/待接入）+ Vue3(JavaScript) + 纯CSS + Celery 5.4 + Redis 7 + PostgreSQL 16 |
...
| AI监控 | LangSmith | 待接入 | 调试追踪 | 目标能力；当前不属于后端直接依赖 |
...
| WSGI服务器 | Gunicorn | 待部署方案确认 | 生产服务 | 目标部署组件；当前本地使用 Django 开发服务 |
...
> 2026-09-18 依赖台账核对：当前直接依赖以 `backend/requirements.txt` 和 `frontend/package.json` 为准；LangSmith 为 LangChain 的间接安装包，Gunicorn 未安装且当前本地/受控启动脚本未使用。LlamaIndex、MCP、HITL 和真实 PostgreSQL/pgvector RAG 仍按各自任务门禁推进，不因架构表出现而标记为已接入。本次仅修正文档口径，未新增依赖或配置。
...
用例生成的 JSON 结果同时保存兼容旧接口的 `type`（positive/negative/boundary/linkage）和规范化的 `test_design_method` / `test_design_method_label`。测试设计方法至少覆盖等价类、边界值、错误猜测法、因果图、状态转换、安全、性能和回归兼容；`source_test_point_id` 必须属于本次已审核集合。模型路由可用但调用失败时，生成服务使用确定性基线完成范围闭环，并在 `coverage_report.analysis_method` 标记 `deterministic_fallback`、逐轮记录错误码和告警。
...
### 11.3 Skill链与分层配置架构（阶段29，对应PRD 11.6）

#### 领域对象

- `Skill` 是可复用的单项能力定义，由平台生成稳定的 `skill_id`，声明触发条件、工具/知识依赖以及输入和输出结构；编辑单个 Skill 只修改该能力定义，不会自动把它接入某项业务流程。
- `SkillInstallation` 保存第三方来源、版本/摘要、审批/权限和安装生命周期证据，不是能力定义；一个安装可发现多个能力。平台按稳定来源身份及包内能力身份自动关联能力库条目，升级沿用 `skill_id` 并保留版本；同名异源条目不静默覆盖。状态分别表达“已安装”和经适配器、契约、依赖、授权及运行环境预检后的“运行就绪”。
- `SkillChain` 是带版本的编排契约；首次版本无回滚目标；回滚必须指向明确的历史已发布版本并派生新版本，目标不存在时返回`no_rollback_target`且前端显示禁用原因；每个链蓝图创建需写初始基线审计。
- `SkillChainConfiguration` 将链引用到全局、项目、功能或即时范围，并记录范围约束、版本锁定和覆盖字段。配置的启停/回滚只作用于范围解析；配置快照保存当时引用的链版本及定义，后续改链不能改变已经保存的历史快照。配置回滚不等于蓝图版本回滚。

#### 逐任务实现边界

1. T167 建立并验证上述领域契约、兼容数据结构、REST CRUD 与持久化快照；它不解析配置优先级，也不执行 Skill 链。技能库中的“编辑”维护单个 Skill；“编辑链契约”只维护链的结构定义。
2. T168实现蓝图版本治理和全局→项目→功能→即时指定的优先级解析、字段合并、版本锁定、配置启停/回滚、权限审计和生效预览；蓝图回滚派生新版本，无历史目标时禁用并说明；蓝图暂停阻止新运行但不暂停已有运行；配置预览/启用不等于执行。
3. T169实现统一运行时：执行开始时固定Skill、模型、提示词、知识库、工具、外发和成本策略快照；按节点Schema与依赖执行，并提供幂等、检查点暂停/恢复、取消、超时、重试和血缘记录。暂停/恢复/取消属于单次运行控制，和蓝图/配置启停分离；暂停必须确认安全检查点，恢复重新校验权限但沿用原输入及配置快照。执行结果必须进入下游业务流程，不能把“契约保存成功”显示成“Skill已执行”。
4. T169A基于统一运行时为本地 `SKILL.md` 提供受控解析与适配；仅消费经过校验的说明/声明并调用平台允许的能力，不执行文件中的任意代码或命令。不能通过运行预检时保持未就绪并阻断。
5. T169B建立真实、可持久化的节点进度快照/事件和通用前端进度视图；T170–T173完成需求/测试点/用例的真实Skill纵向链；T173A统一两个生成入口并在父运行中解析完整链路，区分执行、复用和人工门禁；T174处理来源失效与下游消费；T175-T176完成适配、安全及成本治理；T177验收阶段29。T178-T184按UI/API/性能设计、实际执行器及跨领域验收接入同一进度契约。

#### 前端信息架构与反馈

Skills 工作台必须区分“安装记录”（来源、审批与安装生命周期）、“能力库”（标准化的单个 Skill 定义）、“链契约”（节点和依赖蓝图）、“链配置”（在哪个范围/功能生效）和“运行记录”（真实执行证据）；安装记录与能力库由平台自动关联，普通用户按显示名称选择，不手写 `skill_id` 或 JSON，并分别看见“已安装”和“运行就绪”。每个编辑入口须说明编辑对象、影响范围、是否会执行以及如何撤销；链契约编辑器提供可复制示例、字段说明和校验反馈。T167 页面只宣称契约已保存/校验，不显示尚未实现的生效解析或运行成功；蓝图/配置治理随T168、运行入口随T169提供。


#### 统一执行进度投影与工作流扩展（T169B、阶段30）

#### 业务域独立父级运行与跨域边界

`SkillChainRun` 是统一运行协议的技术实现，但不是把所有测试领域混在一起的全局总运行。每个功能入口必须携带明确的 `workflow_key` 和 `domain`，为需求分析、接口自动化、UI 自动化、App 自动化、性能测试等业务域分别创建独立的父级运行。它们共用快照、血缘、状态机、事件和进度投影实现，但各自维护节点集合、输入输出、必需节点分母、运行 ID、权限范围和完成条件。

不同业务域默认不得互相挂载节点或共享当前运行的百分比。跨域依赖只有在用户明确发起跨域编排时才允许建立，并通过显式的上层运行或受控 `parent_run_id`/`child_run_id` 关系表达参与域、来源产物和边界；接口自动化页面不能读取 UI 自动化父级运行的“最新结果”作为当前输入，UI、App、性能和需求分析同理。单独业务域页面只能查询自己的 `workflow_key`、`domain` 和运行权限范围。

输入类型、领域执行器和 Skill 链由工作流定义声明。例如接口自动化可以消费 OpenAPI 并输出请求/断言/清理资产，UI 自动化可以消费页面流程并输出定位/步骤/断言资产，App 自动化可以消费设备和交互资料并输出移动端执行资产，性能测试可以消费 SLO/负载方案并输出压测计划和指标结果；这些产物必须有不同的 Schema 和版本指纹，不能仅用一个通用 JSON 或标题完成跨域关联。

#### 总运行编排与权威结果边界

跨多个业务节点的入口必须先经过当前业务域的父级运行编排器，不能由前端串联多个 REST 请求，也不能由某个业务 View 旁路调用下游 Service。编排器的最小流程为：权限与运行就绪预检 → 校验 `workflow_key`/`domain`/输入类型 → 解析并校验当前业务域完整 DAG → 冻结执行计划与配置快照 → 创建幂等父级运行 → 按拓扑顺序执行节点 → 校验并持久化节点产物 → 更新节点/父级状态和有序事件。

父级运行的运行快照必须包含业务域、功能入口、输入资料/环境版本、来源指纹、Skill 链和配置版本、节点定义及依赖、领域执行器、模型/提示词/知识库/工具/外发/成本策略、人工门禁、失败策略和幂等键。执行计划一旦进入运行中不可被后台配置变化改写；配置或 Skill 的新版本只能影响新运行。

节点执行必须使用当前业务域拓扑排序后的唯一 `node_id`。预检阶段发现重复节点、未知依赖、环、多个候选 Skill/执行器、输入类型不匹配、环境未就绪或无法解析上游关系时，当前业务域父运行在产生业务副作用前进入 `blocked`，并持久化可定位的错误；不能选择“第一个可用结果”继续执行。节点输出使用不可变版本化产物引用，至少带 `artifact_id`、`artifact_version`、`source_fingerprint`、`producer_run_id`、`producer_node_id`、`workflow_key` 和 `domain`。下游只接受当前业务域父级运行中、通过契约与质量校验的精确产物引用。

状态更新必须遵守持久化顺序：业务产物和证据先成功提交，节点才能进入 `completed`；当前业务域所有适用必需节点完成、人工门禁通过且父级输出快照落库后，该业务域父级才能进入 `completed`。其他业务域的成功不能推进当前业务域父级的完成。失败、超时、取消、供应商/设备/浏览器异常、Schema/权限/质量门禁失败都必须保留节点和父级失败证据，不能用空结果、旧结果或旁路结果补齐。重试默认复用原父级的输入和配置快照，仅增加节点尝试与事件；若输入、Skill、配置、执行器、环境或上游版本变化，必须创建新的业务域父级运行并使受影响下游失效。

同一业务域、同一幂等键只能对应一个活动父级运行。重复点击、页面刷新、Celery 重投和 MCP 重试必须返回或恢复同一运行，而不是创建并行运行或重复计费；用户明确选择“重新执行”时才创建新的尝试，并与旧运行建立 `supersedes`/来源关系。任何旧运行结果都不能被新运行覆盖，当前页面必须明确显示当前业务域的权威运行版本。

Skills执行实例维护权威的节点状态与有序进度事件；事件关联运行ID、工作流类型、链契约版本、节点ID/顺序、权重、当前Skill名称/版本、状态、可恢复序号、提示语键、脱敏错误摘要和实际工作量。持久化运行快照是刷新恢复的读取来源，实时订阅只传递状态变化，不成为唯一数据副本。入口须按项目和执行权限过滤快照及事件。

进度投影按“已完成节点权重 + 运行时可证明的当前节点进度”计算，节点等权为缺省配置；没有可靠的节点内计量时只在阶段边界推进，并同屏显示节点序号/总数。进度不按时间插值，不从模型文本推断。必需节点全部完成且无人为门禁待处理才进入100%；等待人工、排队、失败、取消、超时、重试和因上游变化回退均为独立状态。

需求或用例入口触发时，运行时先固定完整来源链和执行计划；有效且已批准的上游产物作为带版本指纹的复用节点显示，缺失/失效节点按配置继续执行或在人工门禁处暂停。完整血缘展示与本次运行百分比严格分离，复用来源不冒充本次执行。前端提供共享进度条和阶段轨迹组件，使用工作流/阶段键读取平台提示语目录。业务执行器提交阶段键和提示语键；需求分析、需求评审、需求拆解、测试点拆解、测试点评审、用例编写、用例评审及人工门禁提供首批适配。未来UI/API/性能执行器只声明各自的工作流节点和真实进度，复用快照/API/订阅/状态投影及界面，不在前端复制一套进度算法或把阶段文案写成页面判断分支。

T169B实现需求和用例工作台通用视图；阶段30按T178-T184分别完成UI自动化设计/Playwright UIExecutor、接口自动化设计/APIExecutor、性能方案/PerfExecutor和跨领域验收。

#### 跨运行分析稳定性与权威快照

同一来源资料的再次分析必须被实现为新的候选运行，不能把模型本次输出直接写回当前业务记录并覆盖上一版结果。运行时至少要区分 `AnalysisRun`、候选分析产物、`AuthoritativeSnapshot`、跨运行差异报告和人工决策记录；当前权威快照只有在稳定性门禁及人工门禁均通过后才能切换。

分析对象的身份必须由解析证据 ID/内容指纹、父级对象、业务类型、规范化语义和必要的测试类型共同确定。模型生成的临时 ID、名称措辞、数组顺序和数量不能作为唯一主键。每次比较必须输出稳定的对象级差异，包括保持不变、文字变化、新增、疑似删除、父级变化、类型变化、证据变化、语义冲突、重复和无法关联；差异 ID 不能依赖本轮数组序号。

模型未输出对象不等于对象已删除。已确认对象的疑似删除、关键父级或证据变化、覆盖率下降、关键关系断裂及无法唯一关联，必须阻断候选发布并进入人工决策；不得以“最后一次”“数量最多”或“模型置信度最高”自动选为正确版本。相同输入、配置和模型的重复运行出现非文字结构差异时，也必须留下可审计差异报告。

评审、需求拆解、测试点、用例及接口/UI/App/性能资产只能引用明确的权威快照 ID 和版本指纹，不能查询 `latest` 或在运行中改读新版本。权威快照切换后，运行时必须沿血缘精确标记受影响下游为待复核/失效；未受影响的已批准产物可以继续使用并保留其来源。旧数据只能只读兼容并标注证据不足/待复核，不能被伪装成已通过稳定性门禁的新结果。
...
连接测试 API 应返回脱敏的失败阶段，例如 `dns_failed`、`tcp_blocked`、`provider_timeout`、`tls_failed`、`proxy_unavailable`、`provider_http_error`、`auth_failed`、`quota_exhausted` 或 `model_capability_mismatch`；历史 `local_network_blocked` 作为兼容错误码保留，并归入 TCP/本机网络阻断阶段。模型响应超时不得归类为 TCP 阻断。这些错误码由 T146 在 Service、REST Serializer/响应和前端状态之间统一，不得把所有网络问题压缩为一个不可行动的“测试未通过”。健康检查和部署验收必须由与线上相同的 Django/Celery 运行身份执行，不能用浏览器或运维人员电脑的 curl 结果代替。
...
## 22.7 需求分析五轮递进复核运行时【V5.4 / T155C】

需求分析采用“外层语义轮次 + 内层技术分段”的两级编排，不把分段次数当作分析轮次。外层编排器固定执行 5 个语义轮次；每轮复用 `ContextBudgetPlanner`、`StructuredBatchExecutor` 和 `StructuredResultMerger`，根据模型上下文和输出预算产生一个或多个分段。

### 22.7.1 两级执行模型

```text
RequirementDocument + ParseEvidence
          ↓
Round 1 基础拆解
          ↓ 累计结果 + 未覆盖证据 + 问题清单
Round 2 异常/风险补全
...
每轮必须读取完整需求正文、全部解析证据和前轮累计结果；第 2～5 轮还必须携带已覆盖证据、未覆盖证据和前轮问题清单。每轮只输出新增、修正、重复、冲突和未覆盖证据说明，不能静默覆盖有效结果或通过重复项虚增覆盖率。

### 22.7.2 状态与审计数据

- 总体执行状态必须区分 `round_pending`、`round_running`、`partial`、`failed`、`needs_review` 和 `complete`；只有 5 轮均完成且质量门禁通过，才能进入 `needs_review` 的最终测试点审核表。
- `coverage_report` 或等价的审计存储必须分别保存 `round_count=5`、每轮重点、轮次状态、分段轨迹、实际模型、调用次数、本轮新增/修正/重复/冲突数量、累计数量、覆盖率和失败原因。
- 分段失败、输出截断、无效 JSON、供应商错误、证据校验失败或任一轮超时，只能保留已完成轮次并进入可恢复的 `partial/failed`，不得生成完整结果或触发用例生成。
- 跨轮合并必须按稳定业务特征去重，保留首次来源轮次、补充轮次、来源分段和证据 ID；冲突进入人工复核清单。

### 22.7.3 REST 与前端边界

REST 必须返回外层轮次进度和内层分段进度两个维度，前端分别展示“第 N/5 轮”和“本轮第 M/K 段”，并提供失败轮次恢复、历史轮次轨迹和最终审核入口。五轮完成不自动确认需求；前端通过审核动作持久化已审核测试点集合，已审核集合可被用例生成选择器选择，生成服务必须在后端校验输入 ID 属于该集合，不得消费未审核测试点。全部测试点及其模块、功能点、描述、测试类型和原文依据完成最终确认后，结果才标记为完整。

### 22.7.4 预算与安全边界

五轮递进会使模型调用量按“轮次 × 分段数”增长，必须在启动前展示最大分段/调用范围，并受单轮超时、最大重试次数、最大总分段数、费用提示和幂等键限制。不得因模型遗漏而自动补写无证据事实，不得无限重试或静默切换未授权模型。
